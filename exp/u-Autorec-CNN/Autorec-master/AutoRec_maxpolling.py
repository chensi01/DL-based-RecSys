...
class U_AutoRec():
	...
	def prepare_model(self):
		...
		pre_Encoder = tf.nn.sigmoid(tf.matmul(self.input_R, V) + mu)  # pre_Encoder = tf.matmul(self.input_R,V) + mu
		shape_1 = pre_Encoder.get_shape().as_list()#shape  # self.Encoder = tf.nn.sigmoid(pre_Encoder)
		pre_Encoder_1 = tf.expand_dims(tf.expand_dims(pre_Encoder, 0), 3)
		pooling = tf.nn.max_pool(pre_Encoder_1, [1, 1, 50, 1], [1, 1, 1, 1], padding='SAME')
		self.Encoder = tf.squeeze(pooling)#tf.reshape(pooling, shape_1)
	...
	def train_model(self, itr):
		...
		for i in range(self.num_batch):
			...
			_, Cost = self.sess.run(
				[self.optimizer, self.cost],
				feed_dict={self.input_R: self.train_R[:, batch_set_idx].transpose(),
						   self.input_mask_R: self.train_mask_R[:, batch_set_idx].transpose()})
	...
	def test_model(self, itr):
		...
		Cost, Decoder = self.sess.run(
			...
			feed_dict={self.input_R: self.test_R.transpose(),
					   self.input_mask_R: self.test_mask_R.transpose()})
		...
		if (itr + 1) % self.display_step == 0:
			...
			pre_numerator = np.multiply((Estimated_R - self.test_R.transpose()), self.test_mask_R.transpose())
